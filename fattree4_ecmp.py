...
def install_proactive(net, topo):
	...
	for sw in topo.EdgeSwitchList:
		...
		if topo.pod == 4:
			...
			'group_id=1,type=select,bucket=output:1,bucket=output:2'" % sw.name
		elif topo.pod == 8:
			cmd = "ovs-ofctl add-group %s -O OpenFlow13 \
			'group_id=1,type=select,bucket=output:1,bucket=output:2,\
			bucket=output:3,bucket=output:4'" % sw.name
		else:
			pass
		os.system(cmd)
		cmd = "ovs-ofctl add-flow %s -O OpenFlow13 \
		'table=0,priority=10,arp,actions=group:1'" % sw.name
		os.system(cmd)
		cmd = "ovs-ofctl add-flow %s -O OpenFlow13 \
		'table=0,priority=10,ip,actions=group:1'" % sw.name
		os.system(cmd)
